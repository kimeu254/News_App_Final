<template>
    <div>
        <div class="d-flex justify-content-end mb-3">
            <router-link to="/Admin/Categories/Lifestyle/Post" class="btn btn-secondary pull-right">
                Add news
            </router-link>
        </div>
        <div class="mb-4">
        ...
        <div class="table-responsive">
            <table id="example" class="table table-striped table-hover table-condensed" style="width:100%">
                ...
                <tbody>
                    <tr v-for="news in lifestyleCategory" :key="news.id">
                        <td>{{news.id}}</td>
                        <td>{{news.headline}}</td>
                        <td>{{news.story}}</td>
                        <td>
                            <div class="d-flex flex-row">
                                <div class="">
                                    <router-link :to="{name: 'viewLifestyleCatergory', params: {id: news.id}}">
                                        <button class="btn btn-outline-success" data-toggle="tooltip" data-placement="top" title="view"><i class="bi bi-eye"></i></button>
                                    </router-link>
                                </div>
                                <div class="px-2">
                                    <router-link :to="{name: 'editLifestyleCatergory', params: {id: news.id}}">
...
export default {
    ...
    mounted() {
        this.$store.dispatch("getlifestyleCategory").then(() => {
            $('#example').DataTable({
                order: [[0, 'desc']],
            });
        });
        ...
        deletePost(id) {
            this.$swal.fire({
                ...
                if (response.isConfirmed){
                    this.$store.dispatch("removelifestyleCategory", id)
                    this.$swal.fire(
                        {
                            title: 'Deleted!',
