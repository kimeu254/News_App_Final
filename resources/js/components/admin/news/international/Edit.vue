...
export default {
    ...
    computed: {
        news() {
            return this.$store.state.internationalNews.internationalNews.find(f => f.id == this.$route.params.id)
        },
    },
    methods: {
        onChange(e) {
        ...
        async onSubmit() {
            try {
                ...
                const response = await axios.post("/api/internationalNews/"+id, formData)
                .then(res => {
